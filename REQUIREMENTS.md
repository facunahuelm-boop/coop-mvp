...
- **H-SEC-1 (crítico, operativo no de código)**: el aislamiento multi-tenant real depende de que `APP_DATABASE_URL` esté seteada en producción apuntando al rol `app_user` (sin `BYPASSRLS`). Si no lo está, la conexión cae al rol superusuario y **todas** las políticas RLS quedan inertes. Esto ya fue diagnosticado y documentado en `FASE04_SEGURIDAD_DB.md` con instrucciones paso a paso para el usuario; no consta en este audit si ya se ejecutó en producción — **debe reconfirmarse como primer paso antes de tocar nada de Fase 12**.
- **H-SEC-2 (crítico, funcional)**: no existe ninguna ruta de descarga mediada. Todo archivo (documentos, reportes, fotos) se sirve desde un bucket de Supabase Storage **público en lectura**, referenciado por URL directa guardada en la base. Quien obtenga la URL (compartida, en el historial del navegador, por captura de pantalla) puede descargar el archivo sin sesión, sin pertenecer a la cooperativa y sin que el sistema pueda registrar ni impedir el acceso. Ya está señalado en un comentario del propio código (`upload.ts`) como una deuda pendiente. Esto es exactamente lo que la Fase 11 debe resolver: bucket privado + URLs firmadas de corta duración + una ruta `/api` que re-verifique usuario, cooperativa, permiso y entidad antes de cada descarga.
