...
- **H-9**: navegación cruzada persona↔detalle existe solo en 3 lugares (socios, finanzas→socios, proveedores/compras); comisiones, reuniones, obra, gastos, auditoría y mails muestran nombres como texto plano sin enlace. Núcleos familiares no tienen página de detalle propia (`/nucleos/[id]` no existe). 🔶 *(Fase 6, 12/09)*: comisiones y auditoría ahora enlazan el nombre de cada usuario a su ficha (`/usuarios/[id]`, ver sección 5.6) — reuniones, obra, gastos y mails siguen sin enlace; eso, más `/nucleos/[id]`, confirma que la Fase 7 sigue siendo trabajo genuinamente nuevo.
