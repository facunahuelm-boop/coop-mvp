...
### 5.6 Perfil individual de usuario ✅ *(Fase 6, 12/09 — implementado)*

Cubre la mitad de H-13 que Contactos (5.5) no cubría: `users` (cuentas de acceso — comisión, administración, admin) no tenía ninguna página de detalle, a diferencia de `socios/[id]`, que sí es sólida.

1. ✅ Página nueva `/usuarios/[id]` (`src/app/(app)/usuarios/[id]/page.tsx`): nombre, rol, email, fecha de alta, comisiones que integra (`comision_miembros`) y actividad reciente. Usa `socios/[id]` como referencia de estructura (header + tarjeta + secciones), no como contenido a copiar — son conceptos distintos (una cuenta de acceso, no un padrón de personas con vivienda).
2. **Visibilidad**: la ficha básica (nombre, rol, email, comisiones) es visible para cualquier usuario autenticado, sin gate de módulo — mismo criterio de transparencia que ya aplica hoy `/comisiones` (donde estos mismos nombres ya aparecen a todos los roles). La actividad reciente (tabla `auditoria`) sí queda detrás de `canRead(rol, "auditoria")`, salvo que sea la propia persona viendo su propia actividad.
3. **Cambiar contraseña** (`src/lib/actions/usuarios.ts`, `src/components/CambiarPasswordForm.tsx`): agregado porque no existía ningún flujo de autogestión de contraseña en todo el sistema — hueco real, no característica inventada. Solo aparece en la ficha propia; la Server Action nunca recibe un id de usuario por formulario, siempre opera sobre `requireUser()`, así que no hay forma de apuntarla a otra cuenta. Construida con la infraestructura de la Fase 3 (`useActionState` + `FormError`/`FieldError`/`SubmitButton`).
4. **Enlazado desde donde el nombre ya aparecía** (necesario para que la ficha sea alcanzable, no solo la propia): el nombre de cada integrante en `/comisiones` y el autor de cada fila en `/auditoria` ahora enlazan a `/usuarios/[id]`. El acceso a la ficha propia se agregó en el pie de la Sidebar de escritorio (debajo del saludo) y como primer ítem en `/mas` (menú de celular).
5. **Deliberadamente fuera de esta fase**: no hay edición de nombre/rol/estado de otra cuenta, ni alta de usuarios nuevos — eso es "gestión de usuarios" (una pantalla de administración completa, con sus propios permisos y validaciones), alcance mayor al que pide "perfil individual de usuario". Tampoco se resolvió el resto de H-9 (reuniones, obra, gastos, mails sin enlace a persona) — eso queda para la Fase 7 (navegación transversal), que es donde corresponde.
6. Sin cambios de esquema — todas las tablas que usa (`users`, `comision_miembros`, `comisiones`, `auditoria`) ya existían.

### 5.7 Navegación transversal ✅ *(Fase 7, 12/09 — implementado)*

Cierra el resto de H-9: todos los lugares donde un nombre de persona o de núcleo familiar aparecía como texto plano, sin enlace a su ficha.

1. ✅ Componente compartido nuevo `src/components/EntidadLink.tsx` (`UsuarioLink`, `NucleoLink`): renderiza el nombre como texto plano si no hay `id` (fallback configurable), o como `<Link>` a `/usuarios/[id]` o `/nucleos/[id]` si lo hay. Se creó recién en esta fase, no en la 6, porque recién acá el patrón se repite en más de diez archivos distintos — en Fase 5/6 los dos enlaces puntuales (comisiones, auditoría) se dejaron escritos a mano para no tocar código ya correcto sin necesidad.
2. ✅ Página nueva `/nucleos/[id]` (`src/app/(app)/nucleos/[id]/page.tsx`), gateada con `canRead(rol, "trabajo")` (módulo de lectura universal): datos del núcleo (cuota social, horas acumuladas y objetivo semanal, habilidades), socios vinculados (`socios.nucleo_id`, enlazados a `/socios/[id]`), e historial de asistencia a jornadas de trabajo y a reuniones. Resuelve, para el caso de navegación, la mitad de la "Nota de diseño importante" de la sección 3 que la Fase 5 (Contactos) no cubría: cruza `nucleos_familiares` con `socios` sin fusionar los dos modelos — es una vista de referencia, de solo lectura, no una tabla ni relación nueva.
3. **Nombres enlazados** (antes texto plano) en: `/comisiones` (responsable de tarea), `/reuniones/[id]` (responsable de tarea y núcleos en asistencia), `/obra/[id]` (responsable, autor de cada avance, autor de cada problema — este último no se mostraba en absoluto antes), `/gastos` (cargado por), `/reclamos` (reportado por / a cargo de), `/seguridad` (autor, en incidentes e inspecciones), el widget "Actividad reciente" del `/dashboard`, `/mails` (remitente), `/trabajo` y `/trabajo/[id]` (núcleo en asignaciones y en el ranking de horas), y `/socios` + `/socios/[id]` (núcleo del socio).
4. **Deliberadamente sin enlazar, con justificación técnica**: el responsable de tarea en la lista principal `/obra` (no `/obra/[id]`) ya está dentro de una `<Card>` que a su vez es hija de un `<Link>` a `/obra/[id]` — anidar un segundo enlace ahí produciría HTML inválido (ancla dentro de ancla); se dejó como texto plano, mitigado por el enlace ya agregado en el detalle de la tarea. Los destinatarios individuales de un mail en `/mails` no se pueden enlazar: el snapshot guardado en `destinatarios` solo tiene `{nombre, email}`, sin `id` — requeriría un cambio de esquema, fuera de alcance. El campo `autorNombre` de notas de calendario y el join `usuario_nombre` de `alertasEmail` en Configuración están en el código pero no se renderizan en ninguna pantalla hoy (código muerto) — agregarles UI sería una funcionalidad nueva no pedida, no una corrección de un enlace faltante.
5. Es una fase de **solo lectura y navegación**: no se agregaron Server Actions, mutaciones ni cambios de esquema — todas las consultas nuevas son `SELECT` puros contra tablas ya protegidas por RLS, y todos los enlaces van a páginas cuyo propio gate de permiso ya es compatible con los módulos universales de lectura ("socios", "comisiones", "trabajo") desde donde se enlaza.
6. H-9 queda **resuelto en su totalidad** (ver sección 6).

### 5.8 Paginación, búsqueda y filtros ✅ *(Fase 8, 12/09 — implementado)*

Resuelve H-10: listados con un `LIMIT` fijo (15/50/200) que dejaba los registros más viejos permanentemente invisibles, sin ninguna forma de llegar a ellos.

1. ✅ Componente compartido nuevo `src/components/Pagination.tsx`: enlaces puros (`<Link>`, sin JavaScript ni estado de cliente) con el patrón "← Anterior / Página X de Y / Siguiente →", elegido en vez de una lista de números de página por ser lo más simple de interpretar para alguien sin conocimientos técnicos o adultos mayores — menos elementos para leer, blancos de toque más grandes, y funciona igual con JavaScript deshabilitado. Cada página preserva sus propios filtros (querystring) al cambiar de página.
2. **Patrón aplicado en todos los casos**: reemplazar el `LIMIT` fijo por `COUNT(*)` + `LIMIT`/`OFFSET` reales contra la base (nunca un recorte en memoria con `.slice()`, que además traía de la base filas que después se descartaban sin usar).
3. **Listados corregidos**: `/auditoria` (200 → 30 por página, ver punto 4), `/mails` (50 → 20 por página), `/gastos` (200 → 30 por página, mantiene todos los filtros ya existentes de la Fase anterior a esta), `/finanzas` → sección "Movimientos" (15 → 20 por página; se renombra de "Movimientos recientes" porque deja de ser un recorte y pasa a ser el listado completo), `/reuniones` → "Historial" (15 → 15 por página; "Próximas" queda sin cambios por ser una lista naturalmente acotada a lo planificado a futuro, no un historial que crece), `/trabajo` → "Jornadas anteriores" (8 → 10 por página), `/reclamos` → "Resueltos" (antes: traía TODOS los reclamos de la base y recién ahí recortaba a 10 en memoria, con los resueltos más viejos invisibles y sin ninguna forma de verlos; ahora: dos consultas separadas — "abiertos" siempre completa, sin límite, porque un reclamo sin resolver nunca puede volverse invisible — y "resueltos" con paginación real, 10 por página).
4. **Filtros nuevos en `/auditoria`**: por usuario y por tipo de entidad (dos `<select>`, mismo patrón `method="GET"` ya usado en `/gastos`) — esta es la pantalla de auditoría completa del sistema, y encontrar un registro puntual no debería depender de recorrer página por página. Se agrega también un enlace "Ver toda la actividad →" en `/usuarios/[id]` (cuando el usuario que mira tiene permiso de auditoría) que lleva a `/auditoria?usuario_id=X` ya filtrada — antes esa sección de perfil se cortaba en los últimos 20 registros sin ninguna forma de ver más.
5. **Búsqueda nueva en `/socios`**: componente compartido nuevo `src/components/BuscadorFilas.tsx` — buscador 100% del lado del cliente sobre las filas del padrón ya renderizadas por el servidor (mismo criterio que `ContactosLista.tsx` de la Fase 5, que ya preveía este momento en su propio comentario). Se eligió búsqueda client-side y no paginación real porque el padrón de una cooperativa está acotado por su cantidad real de viviendas (decenas de filas, no miles) — no amerita un ida-y-vuelta al servidor por cada letra tecleada. Las filas mantienen sus `<AutoSubmitSelect>` con Server Actions intactos: un Server Component puede pasar JSX ya renderizado (`children`, o cualquier otra prop nombrada) a un Client Component, así que `BuscadorFilas` nunca necesita conocer la forma de cada fila, solo decide cuáles mostrar. **Nota de un bug propio, detectado y corregido antes de cerrar esta fase** (detalle en `CHANGELOG.md`): la primera versión de este componente rompía `/socios` en producción (500, "Minified React error #441") porque, además de JSX, hacía cruzar una *función* corriente desde el Server Component hacia el Client Component (primero como `children` en forma de render-prop, y en un primer intento de arreglo, como un prop `envolver`) — una función común (a diferencia de una Server Action) no puede serializarse a través de ese límite. La versión final no pasa ninguna función: `claves` (texto plano) y `children`/`encabezado` (JSX ya armado) son los únicos props, y quedó verificado en vivo sin errores de consola ni de red.
6. **Deliberadamente sin cambios, con justificación**: los widgets del `/dashboard` (son resúmenes acotados a propósito, cada uno con su propio "Ver X →" hacia la pantalla completa correspondiente — agregarles paginación duplicaría esas pantallas); las consultas de `/calendario` con `LIMIT 40` (son una ventana hacia adelante desde "hoy", no un recorte de historial — nada queda permanentemente oculto, los eventos futuros van apareciendo con el tiempo); la actividad reciente de `/usuarios/[id]` y `/nucleos/[id]` y los avances de `/obra/[id]` (widgets de "actividad reciente" en una ficha de detalle, mismo criterio ya establecido y aceptado en la Fase 6); las "Resueltas recientemente" de `/alertas` (mismo criterio — un widget atenuado y explícitamente etiquetado "recientemente", no el listado principal de la pantalla); `/documentos`, `/proveedores` y `/compras` (ya tienen filtros reales y ninguna consulta con `LIMIT` fijo — exactamente el estado que el propio hallazgo H-10 señalaba como correcto).
7. Sin cambios de esquema — todas las columnas usadas para contar, filtrar y ordenar ya existían.

### 5.9 Sistema de email reutilizable ✅ *(Fase 9, 13/09 — implementado)*

Resuelve H-12: `enviarEmailAlerta` y `enviarEmailPersonalizado` (`src/lib/email.ts`) armaban y mandaban el mail cada una por su cuenta, duplicando por completo el armado del HTML y el manejo de errores.

1. ✅ **De-duplicación real**: se extrajeron `plantillaHtml()` (arma la tarjeta HTML — mismo diseño de antes, ahora en un solo lugar) y `enviarEmailBase()` (toma la config SMTP ya cargada, llama a `sendMail` y devuelve siempre un `ResultadoEnvio`, nunca una excepción sin capturar). `enviarEmailAlerta` y `enviarEmailPersonalizado` ahora solo arman el contenido específico de su caso (asunto, destinatario, texto del cuerpo, pie de página) y se lo pasan a `enviarEmailBase` — ninguna de las dos vuelve a tocar `nodemailer` ni el HTML directamente. Firma pública de ambas funciones sin cambios (mismos parámetros, mismo tipo de retorno) — ningún llamador (`logic.ts`, `actions/mails.ts`) necesitó cambios.
2. ✅ **"Reutilizable" en la práctica**: cualquier nuevo flujo de email desde otro módulo del sistema puede sumar su propia función pública en `email.ts` que llame a `enviarEmailBase()` con su propio asunto/cuerpo/pie de página, sin duplicar la tarjeta HTML ni el try/catch — es la pieza que faltaba para no repetir el mismo problema una tercera vez. No se agregó ningún nuevo punto de envío en otros módulos (por ejemplo, avisar por mail al resolver un reclamo) porque no fue pedido explícitamente y hacerlo ahora sería una funcionalidad nueva no solicitada, no una corrección del hallazgo — queda disponible para cuando se pida.
3. ✅ **Preview de destinatarios en `/mails`** (la otra mitad de H-12 — "sin preview de destinatarios"): antes de mandar, cada `<option>` de comisión ahora muestra cuántos de sus integrantes tienen email cargado (`Comisión de Compras (4 con email)`), el checkbox "Todos" muestra el total de usuarios activos con email, y cada usuario sin email cargado se marca directamente en su propia opción (`Fulano (sin email — no se le puede mandar)`) — sin agregar JavaScript, con las mismas tres consultas que ya se hacían más el `COUNT` correspondiente. Antes, la única forma de enterarse de que a alguien no le llegaba nada era el mensaje de error después de intentar mandarlo.
4. Sin cambios de esquema ni de comportamiento de envío — mismo SMTP, mismo diseño de mail, mismo registro en `mensajes_correo`.

### 5.10 Confirmaciones de borrado con `ConfirmDialog` ✅ *(Fase 10, 13/09 — implementado)*

Resuelve H-11: `Modal`, `ConfirmDialog` y `ToastProvider` (`src/components/ui-client.tsx`) estaban construidos desde una fase anterior del rediseño UI/UX pero nunca se habían conectado a ninguna pantalla real.

1. ✅ **Alcance real, acotado a lo que H-11 señalaba**: se buscó en todo el código el patrón "escribí ELIMINAR para confirmar" — aparece en exactamente 3 lugares (`/documentos`, `/compras/[id]`, `/proveedores/[id]`), los únicos tres borrados permanentes de verdad (`DELETE` real) que tiene el sistema; todo el resto de "eliminar" en la app es baja lógica (`estado`/`activo`), que no usa este patrón y no lo necesita.
2. ✅ **Componente nuevo `src/components/ConfirmarEliminar.tsx`**: mismo criterio de wrapper cliente puntual que `AutoSubmitSelect.tsx` — un botón dispara un `<ConfirmDialog>` con el nombre real de lo que se va a borrar (`Se va a borrar "Casa 12" de forma permanente...`) y recién al confirmar ahí adentro se manda el `<form>` (mismo Server Action de siempre, sin tocarlo).
3. ✅ **La validación de backend NO se tocó, a propósito**: las 3 Server Actions (`eliminarDocumentoAction`, `eliminarSolicitudAction`, `eliminarProveedorAction`) ya validaban server-side que `confirmacion === "ELIMINAR"` — la seguridad real nunca dependió del campo de texto en sí, sino de esa verificación en el servidor. Se mantiene sin cambios (el nuevo componente manda ese mismo valor fijo por un campo oculto): la confirmación humana pasa a ser el modal de dos pasos (click en "Eliminar" → leer la consecuencia → click en el botón rojo dedicado) en vez de escribir una palabra exacta, que no es más segura y es más difícil de acertar para alguien sin conocimientos técnicos o en el celular — es una mejora de usabilidad, no un debilitamiento de ningún control real.
4. **Deliberadamente fuera de alcance de esta fase**: no se conectó `ToastProvider` a ningún layout ni pantalla — hoy no hay ningún flujo concreto que lo necesite sin inventar una funcionalidad nueva no pedida (agregar avisos "Cambios guardados" a formularios que ya muestran su resultado de otra forma sería eso). Queda disponible, ya construido, para cuando una fase futura lo necesite de verdad. Tampoco se tocó ningún otro aspecto visual del sistema (colores, tipografías, layout) — H-11 es puntualmente sobre estos tres componentes sin conectar, no un rediseño general.

### 5.11 Cierre de descargas mediadas ✅ *(Fase 11, 13/09 — implementado; falta un paso manual para el cierre total)*

Completa H-SEC-2: extiende el patrón de `/api/archivos/documento/[id]` y `/api/archivos/reporte/[id]` (ya existentes) a los cuatro lugares que todavía imprimían la URL pública y permanente de Supabase Storage directo en el HTML.

1. ✅ **Auditoría primero**: se buscó `archivo_url|foto_url|comprobante_url` en toda la app antes de tocar nada. Resultado: 3 fotos sin mediar (`obra/[id]` avances, `seguridad` incidentes, `reclamos`) y un caso más grave encontrado de paso — `gastos_comision.comprobante_url` se guardaba desde el alta del gasto pero **no se mostraba en ningún lugar**, ni mediado ni directo: quien cargaba un comprobante no tenía forma de volver a verlo. Se cuenta como parte de H-SEC-2 (es el "comprobante de Gastos" que el hallazgo ya nombraba) y no como funcionalidad nueva no pedida: sin esto, la fase no cerraba lo que el propio hallazgo pedía.
2. ✅ **4 rutas mediadas nuevas**, mismo patrón exacto que las 2 ya existentes (sesión → `canRead` del módulo correspondiente → fila vía `get()`, ya filtrada por `organization_id` por RLS → `getSignedUrl()` de 120s → redirect): `/api/archivos/avance-obra/[id]` (`canRead "obra"`), `/api/archivos/incidente-seguridad/[id]` (`canRead "seguridad"`), `/api/archivos/reclamo/[id]` (`canRead "reclamos"`), `/api/archivos/gasto/[id]` (`canRead "compras"` O `"finanzas"`, igual que el gate de lectura de la propia pantalla `/gastos`, para no ser ni más ni menos permisivo que la pantalla que enlaza ahí).
3. ✅ **Cero cambios de interfaz**: en Obra/Seguridad/Reclamos, las fotos se seguían mostrando como `<img>` inline (no un link para descargar) — se preservó exactamente así, solo cambiando el `src` a la ruta mediada; el navegador sigue la redirección 302 de forma transparente y la miniatura se ve idéntica a antes. Para Gastos, que no tenía ninguna forma de ver el comprobante, se agregó un link de texto "Ver comprobante →" con el mismo estilo ya usado en otros links secundarios de la pantalla (ej. "Exportar CSV →"), mínimo y consistente con lo existente.
4. ✅ **`logo_url` de la cooperativa (Configuración, login) queda deliberadamente fuera**: se muestra en la pantalla de login, antes de cualquier sesión — mediarlo detrás de una verificación de sesión rompería el login (no se puede exigir sesión para mostrar el logo de la pantalla que sirve para iniciar sesión). No es información sensible ni específica de un socio, así que se lo deja público a propósito, no por omisión.
5. ⚠️ **Pendiente, no resoluble desde el código de este sistema**: el bucket `uploads` de Supabase Storage sigue configurado como público en lectura. Con el punto 1-4 de esta fase más lo ya hecho en la fase anterior, **ya no queda ninguna pantalla que imprima una URL directa y permanente** — es la condición que faltaba para poder pasar el bucket a privado sin romper nada. Pasarlo a privado requiere una acción manual en el panel de Supabase (Storage → bucket `uploads` → Settings → desactivar "Public bucket") o programáticamente con la Service Role Key (`supabase.storage.updateBucket("uploads", { public: false })`) — ninguna de las dos está disponible desde este entorno de trabajo (no tiene las credenciales de producción de Supabase). Una vez hecho ese cambio, ninguna URL vieja ya compartida/guardada por alguien seguirá funcionando, y todo archivo pasará a depender exclusivamente de las rutas mediadas de arriba — comportamiento ya soportado sin cambios de código adicionales (`getSignedUrl()` genera igual la URL firmada esté el bucket en público o privado). Recomendación: hacerlo cuanto antes ahora que el código ya no depende de la URL pública en ningún lugar.
...
- **H-SEC-2 (crítico, funcional) — RESUELTO EN CÓDIGO, falta 1 paso manual de infraestructura (13/09)**: no existía ninguna ruta de descarga mediada; todo archivo se servía desde un bucket de Supabase Storage público en lectura, por URL directa guardada en la base. Fase 11 (sección 5.11) extendió a Obra/Seguridad/Reclamos/Gastos el patrón mediado que Documentos/Reportes/Reuniones ya tenían desde antes — hoy **ninguna pantalla de la app imprime una URL directa y permanente** (la única excepción deliberada es el logo de marca en el login, público a propósito). Lo único que falta para el cierre total es una acción de infraestructura fuera del alcance de este entorno de trabajo: pasar el bucket `uploads` a privado en el panel de Supabase (instrucciones exactas en la sección 5.11, punto 5) — el código ya soporta ese cambio sin modificaciones adicionales. Detalle en `CHANGELOG.md`.
...
- **H-5** ✅ *(Fase 3, 12/09)*: el mismo error de "tabla faltante" (Postgres 42P01) se manejaba de tres formas distintas según el archivo. Se centralizó en `relanzarConMensajeSiFaltaTabla()` (`src/lib/db.ts`), que reemplaza los dos cierres locales duplicados de `calendarioNotas.ts` y `gastos.ts` (comportamiento unificado: mensaje claro + registro en auditoría). El resto de los usos de esta lógica en otros módulos se revisa a medida que se conviertan sus formularios (ver H-6).
- **H-6** 🔶 *(Fase 3, en progreso)*: ninguna acción del sistema usaba `useActionState` salvo el login. Se construyó la infraestructura general (`src/lib/actionState.ts`: tipo `ActionState`, `conEstadoDeAccion()`; `FormError`/`FieldError`/`SubmitButton` en `src/components/ui-client.tsx`) y se convirtió el primer formulario de punta a punta como caso de prueba: **notas de calendario** (crear/editar/borrar, `src/lib/actions/calendarioNotas.ts` + `src/components/MonthCalendar.tsx`) — elegido primero porque hoy mismo hace crash en producción por la migración `notas_calendario` pendiente (H-4), y con esto pasa a mostrar un mensaje claro en el propio formulario en vez de la pantalla genérica. Los formularios restantes del resto de los módulos se van a ir convirtiendo con el mismo patrón en los próximos pasos de esta fase (progreso incremental y documentado, no de una sola vez).
- **H-7** ✅ *(Fase 3, 12/09)*: al revisar el código real (no solo el hallazgo original de la auditoría), `reportes.ts` ya usaba correctamente `redirect("/login")` solo para "sin sesión" y `throw new Error(...)` para "sin permiso" — el hallazgo original estaba mal atribuido a ese archivo. El problema real estaba únicamente en `configuracion.ts` (5 acciones), que sí conflacionaba ambos casos en un solo `redirect`. Se agregó `requireAdminOConsejo()` en ese archivo, que separa los dos casos igual que el resto del sistema.
...
- **H-9**: ✅ *(Fases 6 y 7, 12/09)* navegación cruzada persona↔detalle existía solo en 3 lugares (socios, finanzas→socios, proveedores/compras); comisiones, reuniones, obra, gastos, auditoría y mails mostraban nombres como texto plano sin enlace, y núcleos familiares no tenían página de detalle propia. Fase 6 resolvió comisiones y auditoría, más la ficha `/usuarios/[id]` en sí (sección 5.6). Fase 7 resolvió el resto: reuniones, obra, gastos, reclamos, seguridad, dashboard y mails enlazan al usuario correspondiente, `/trabajo`+`/trabajo/[id]`+`/socios`+`/socios/[id]` enlazan al núcleo, y se agregó `/nucleos/[id]` (sección 5.7). Quedan dos excepciones documentadas con justificación técnica, no pendientes de esta fase: el responsable de tarea en el listado principal de `/obra` (restricción real de HTML anidado) y los destinatarios individuales de un mail en `/mails` (el dato no tiene `id` guardado).
- **H-10**: ✅ *(Fase 8, 12/09)* no existía paginación en ningún listado — todos usaban `LIMIT` fijo (15/50/200), lo que hacía invisibles los registros más viejos en vez de paginarlos. Los únicos filtros reales que ya existían eran: tags en documentos, tabs de estado en proveedores, y filtros multi-campo por querystring en gastos. Se agregó paginación real (`COUNT` + `LIMIT`/`OFFSET`) en auditoría, mails, gastos, finanzas, reuniones, trabajo y reclamos, filtros nuevos en auditoría (usuario, entidad), y búsqueda client-side en el padrón de socios — detalle completo en la sección 5.8.
- **H-11**: ✅ *(Fase 10, 13/09)* `Modal`, `ConfirmDialog` y `ToastProvider` ya estaban construidos en `src/components/ui-client.tsx` pero no estaban conectados a ninguna pantalla. El patrón de confirmación de borrado real era "escribí ELIMINAR para confirmar" en un `<details>` — se reemplazó por `ConfirmDialog` (la opción que el propio hallazgo recomendaba). Detalle en la sección 5.10.
- **H-12**: ✅ *(Fase 9, 13/09)* no existía ningún componente ni flujo reutilizable de envío de email — `/mails` era la única puerta de entrada, sin preview de destinatarios ni invocación desde otros módulos. Además, el envío manual (`enviarEmailPersonalizado`) y el de alertas automáticas (`enviarEmailAlerta`) duplicaban por completo la lógica de armado/envío, compartiendo solo la configuración SMTP. Detalle en la sección 5.9.
- **H-13**: ✅ *(Fases 5 y 6, 12/09)* no existía ninguna sección "Contactos" ni página de perfil de usuario genérica — sí existía una página de perfil de socio sólida (`socios/[id]`), usada como referencia de estructura. Contactos: sección 5.5. Perfil de usuario: sección 5.6.
